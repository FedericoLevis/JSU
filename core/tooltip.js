...
<b>Description:</b>        tooltip API:   Tip* UnTip*   <BR/>   
<b>REQUIRE:</b>            nothing   <BR/>
<b>OPTIONAL:</b>	<BR/>	
				- rainbow: to use TipJSFixedClicked  <BR/>  
				- jslog: to use jslog (if not defined tt_log simply will not do anything)  <BR/>
...
/**
 * NOTE: Not all the Browser support Rainbow (alos depending on security settings).
 * therefore we manage Rainbow call with try catch and we show JS not hightlighted if Rainbow is not supported
 * 
 * Create a FixedTip for a Code Sample: a Table with codeHihtlighted, a Gif and optional Link
 * 
 * @param event
...
function TipFixedCodeSample(event, jsCode,videoEmbedUrl, objOpt){
  ...
  var bRainbow = true;
	if (typeof (Rainbow) != "object"){
		bRainbow = false;
		tt_log ("Rainbow not present: we simply show JS not higthlighted");
		objOpt.bMsgHtml = false;
	}else{
		var div = tt_jsCode2RainbowDiv (jsCode, objOpt);
		// Check with try/catch if Rainbow is supported by Current Browser/settings
		try {
			Rainbow.color(div, function() {
			  ...
			  '        <tr><td align="left">' +   div.innerHTML + '</td></tr>' + szTrUrl +
			  '      </table>' +
			...
			tt_log("We simply show JS not higthlighted because this Browser dows not support Rainbow - Rainbow Exception: " + err.message);
			objOpt.bMsgHtml = false;
			bRainbow = false;
		}
	}
	if (!bRainbow){
		// Particular cases for Browsers not supporting Rainbow
		var szTable  =  '<table class="tip" BORDER="2" cellspacing="0">';
	  var szTip = szTable +
	  '  <tr class="tiptitle">' +
	  '    <td class="tiptitle">' + "JS CODE SAMPLE" + '</td>' + 
 ...
 * @param [objOpt] {Object}   
 * 														iJSMaxHeight:  Max Height (px) of the div that contain the JS   (with autoscroll)
 * 														iJSMaxWidth:  Max Width  of the div that contain the JS         (with autoscroll) 
 * 														     
 * @returns {Object} div to use in Rainbow function
 */
function tt_jsCode2RainbowDiv(jsCode,objOpt){
	var Fn = "[tooltip.js tt_jsCode2RainbowDiv] ";
	tt_log (Fn + "--- START");
	if (typeof (Rainbow) != "object"){
		return tt_Err("To use JS Code tooltip.js Features, you need rainbow.js and the relative js/css !");
	}
	var div = document.createElement('div');  
	var szMaxHeight = "", szMaxWidth = "";
	var bDivScroll = false;
	if (objOpt){
	  if (objOpt.iJSMaxHeight){
		  szMaxHeight = 'max-height: ' +objOpt.iJSMaxHeight + 'px;';
		  bDivScroll = true;
	  } 
	  if (objOpt.iJSMaxWidth){
		  szMaxWidth = 'max-width: ' +objOpt.iJSMaxWidth + 'px;';
		  bDivScroll = true;
	  } 
	}
	var szDivHTML = "";
	if (bDivScroll){
		// Add Div for scroll
		// e.g "<div style='max-height: 200px;overflow: auto;'>"
		szDivHTML = "<div style='" + szMaxHeight + szMaxWidth +    "overflow: auto;'>";
	}
	szDivHTML = szDivHTML +	 "<pre><code data-language='javascript'>" + jsCode + '</code></pre>';
	if (bDivScroll){
		szDivHTML += "</div>";
	}
	div.innerHTML = szDivHTML;
	tt_log (Fn + "--- END");
	return div;
...
/**
 * NOTE: Not all the Browser support Rainbow (also depending on security settings).
 * therefore we manage Rainbow call with try catch and we show JS not hightlighted if Rainbow is not supported
 * 
 * @param jsCode  {String}  jsCode to display, with \n for newline
 * @param event
...
function TipJSFixedClicked(jsCode, event, objOpt){
	...
	if (objOpt.bCloseBtn == undefined){	objOpt.bCloseBtn = TIP_DEF_CLOSE_BTN; }
	tt_init(); // init, if not already done
	// prepare szJsTxt for case of Rainbow not supported
	var szJsTxt='<textarea rows="15" cols="150" readonly>' + jsCode + '</textarea>';
	if (typeof (Rainbow) != "object"){
		tt_log ("Rainbow not present: we simply show JS not higthlighted");
		objOpt.bMsgHtml = false;
		return TipFixedClicked (szJsTxt,event,objOpt);
	}
	var div = tt_jsCode2RainbowDiv (jsCode, objOpt);
	// Check with try/catch if Rainbow is supported by Current Browser/settings
	try {
		Rainbow.color(div, function() {
			TipFixedClicked (div.innerHTML,event,objOpt);
		});	
	}catch(err){
		tt_log("We simply show JS not higthlighted because this Browser dows not support Rainbow - Rainbow Exception: " + err.message);
		objOpt.bMsgHtml = false;
