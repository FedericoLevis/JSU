...
function pp_Init(){
  ...
  if (elPopup == null){
    ...
    jQuery('body,html').append (POPUP_DIV_EMPTY);
  }
...
function pp_ClassInit(szPopupType, jqePopup){
  ...
  elTitleBar.style.backgroundColor =  jQuery('#PopupTblHea').css( "background-color" );
  elTitleBar.style.textAlign =  "center";
  elTitleBar.style.fontSize = jQuery('#PopupTblHea').css( "font-size" ); 
  elTitleBar.style.fontWeight= jQuery('#PopupTblHea').css( "font-weight" ); 
  elTitleBar.style.color= jQuery('#PopupTblHea').css( "color" );
  //---------------
...
function pp_ValidateInput(elInput){
  ...
  return jsu_log(fn + JSU_LOG_FUN_END);
}
 ...
 * @param [objOpt] {Object}   
 <table class="jsDoc" border="2" cellpadding="2" cellspacing="2" width="100%">
  ...
  <ul>
    ...
    <li> iWidth:  {Number}      Optional PopupWidth: if it passed it is used<BR/>
         Else Default is used </li> 
    <li> iHeight:  {Number}     Optional PopupHeight: if it passed it is Set<BR/>
        Else is automarically calculated </li> 
